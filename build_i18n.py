...
NODE_S={
 ...
 'uk':{'pattern':'стискається моделлю','attractor':'стійка зона у просторі станів','form':'атрактор, втілений у матерії',
  'agency':'захищений патерн','mind':'моделює можливі патерни','compression':'патерн як опис',
  'dynstab':'патерн як повернення','time':'патерн, що розгортається','compute':'клітини теж обчислюють',
  'beauty':'поступ стиснення','possible':'те, що могло б бути · платонізм?','logic':'перший фільтр: несуперечність',
  'isreal':'відтворюване + передбачає','reality':'дійсність чинить спротив','existence':'патерн, що триває',
  'attrtypes':'доля клітини · фенотипи · нейротипи?','ideaattr':'патерни у просторі ідей',
  'scale':'локальний атрактор, шкода для цілого','selection':'що втілювати?','ourrole':'шукати·перевіряти·обирати·втілювати'},
...
EDGE_L={
 ...
 'uk':['стає стійким','втілюється в матерії','захищається під тиском','моделюється','постає як','постає як','розгортається у','працює на',
  ...
  'живе в розумах','різновид атрактора','мусить обирати','лягає на нас','змушує обирати'],
...
add('ladder.full',
 ...
 '<li><b>Агентність</b> &mdash; <span>дієве збереження форми чи цільового стану під збуренням.</span></li>'
 ...
 '</ul><p>Каждая ступень добавляет то, чего нет у предыдущей. Потому лестница лучше лозунга &laquo;всё есть паттерн&raquo;, который не объясняет ничего. Об одном слове, что вернётся дальше, стоит предупредить. В строгом смысле &mdash; том, который уточняет нелинейная динамика, &mdash; <em>аттрактор</em> требует пространства состояний и динамики: множества, к которому сходятся траектории. Буквально я беру его лишь там, где их можно задать; для разумов и культур &mdash; как выверенную аналогию.</p>')
...
add('logic.full',
 ...
 '<h2>Перший фільтр</h2><p>Перш ніж фізика щось звузить, логіка вже окреслила зовнішню межу: квадратне коло ніде не чекає, щоб його збудували. Несуперечність &mdash; це перший фільтр на просторі патернів. Далі фізика лишає стійкі, матерія втілює декотрі з них, а агентність обороняє небагато.</p>'
...
add('compute.full',
 ...
 '<p>Що його більше, то ширше коло досяжного. Більше обчислень &mdash; ширший огляд простору патернів, а отже, глибші знайдені стиснення й більше видобутої цінності; і більше агентності теж, адже багатша модель відповідає на більше різновидів збурення. Закон зростання той самий, чи то субстрат &mdash; мікросхема, чи організм. Обчислення не вигадує можливостей; воно обстежує, втілює й обмежує ті, що субстрат уже дозволяє.</p>',
